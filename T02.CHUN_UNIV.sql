...
-- 13.
SELECT T.CLASS_NAME, T.DEPARTMENT_NAME
FROM (SELECT CLASS_NAME, DEPARTMENT_NAME, C.CLASS_NO
      FROM TB_CLASS C, TB_DEPARTMENT D
      WHERE C.DEPARTMENT_NO = D.DEPARTMENT_NO
        AND D.CATEGORY = '예체능') T, TB_CLASS_PROFESSOR CP
WHERE T.CLASS_NO = CP.CLASS_NO(+)
...
-- 15.
SELECT S.STUDENT_NO 학번, S.STUDENT_NAME 이름, D.DEPARTMENT_NAME "학과 이름", ROUND(T.평점, 6) AS 평점
FROM (SELECT G.STUDENT_NO, SUM(G.POINT)/COUNT(G.POINT) AS 평점
      FROM TB_GRADE G
      WHERE G.STUDENT_NO IN (
      SELECT STUDENT_NO
      FROM TB_STUDENT
      WHERE ABSENCE_YN = 'N')
      GROUP BY G.STUDENT_NO) T, TB_STUDENT S, TB_DEPARTMENT D
WHERE T.STUDENT_NO = S.STUDENT_NO(+)
  AND S.DEPARTMENT_NO = D.DEPARTMENT_NO(+)
  AND 평점 >= 4
ORDER BY S.STUDENT_NO;
-- 16.
SELECT C.CLASS_NO, CLASS_NAME, ROUND(POINT, 8) AS "AVG(POINT)"
FROM TB_CLASS C, (SELECT CLASS_NO, SUM(POINT)/COUNT(POINT) AS POINT
                  FROM TB_GRADE
                  GROUP BY CLASS_NO) T
WHERE C.CLASS_NO = T.CLASS_NO
...
FROM TB_STUDENT S, (SELECT STUDENT_NO, SUM(POINT)/COUNT(*) POINT
                    FROM TB_GRADE
                    ...
                    GROUP BY STUDENT_NO
                    ORDER BY POINT DESC) T
WHERE S.STUDENT_NO = T.STUDENT_NO
  AND ROWNUM = 1;
-- 19.
WITH DEPT_NAME AS (SELECT T.DEPARTMENT_NAME, C.CLASS_NO
                    FROM TB_CLASS C, (SELECT DEPARTMENT_NO, DEPARTMENT_NAME
                                      FROM TB_DEPARTMENT
                                      WHERE CATEGORY = (
                                      SELECT CATEGORY
                                      FROM TB_DEPARTMENT
